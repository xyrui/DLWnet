...
parser = argparse.ArgumentParser(description = 'Training HWNUCLR & HWTV')
parser.add_argument('--ps', dest = 'patch_size', default = [64,31])
parser.add_argument('--bs', dest = 'batch_size', default = 10)
parser.add_argument('--save_path', dest = 'save_path', default = './cks/NUCLR_TV', help = 'pretrained models are saved here')
...
parser.add_argument('--mn', dest='model_name', default='NUCLR_TV_Complex')
...
if __name__ == '__main__':
    ...
    for ep in range(epoch_start, args.epoch):
        ...
        for ii, t_p in enumerate(Train_dataset):           
            ...
            loss = (1/2)*loss_nuclr + (1/2)*loss_tv 
            ...
            if (ii+1)%20 == 0:
                logger.info(f"Batch [{ii+1:4d}]/[{Batch_group:4d}][{ep+1:4d}] -- Loss: {loss.item():.3e} -- L_tv: {loss_tv.item():.3e} -- L_nuclr: {loss_nuclr.item():.3e}")
            
        lr_scheduler.step()    
            
        toc = ti.time()
        
        logger.info(f"----- Epoch [{ep+1:4d}]/[args.epoch:4d] -- Loss: {writer_loss/ Batch_group:.3e} -- L_nuclr: {writer_l_nuclr/ Batch_group:.3e} -- L_tv: {writer_l_tv/ Batch_group:.3e} -- Time: {toc - tic:.4f}")
        print('#######################')
