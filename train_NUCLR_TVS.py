...
from net import HWNUCLR, HWTV_S
...
parser = argparse.ArgumentParser(description = 'Training HWNUCLR & HWTVS')
parser.add_argument('--ps', dest = 'patch_size', default = [64,31])
parser.add_argument('--bs', dest = 'batch_size', default = 10)
parser.add_argument('--save_path', dest = 'save_path', default = './cks/NUCLR_TVS', help = 'pretrained models are saved here')
...
parser.add_argument('--mn', dest='model_name', default='NUCLR_TVS_Complex')
...
if __name__ == '__main__':
    ...
    for ep in range(epoch_start, args.epoch):
        ...
        for ii, t_p in enumerate(Train_dataset):           
            ...
            pred_tvs = netD_tvs(Binput.permute(0,2,3,1), W.permute(0,2,3,1)).permute(0,3,1,2)
            
            loss_nuclr = F.mse_loss(pred_nuclr, Blabel)
            loss_tvs = F.mse_loss(pred_tvs, Blabel)
            
            loss = (1/2)*loss_nuclr + (1/2)*loss_tvs
            ...
            if (ii+1)%20 == 0:
                logger.info(f"Batch [{ii+1:4d}]/[{Batch_group:4d}][{ep+1:4d}] -- Loss: {loss.item():.3e} -- L_nuclr: {loss_nuclr.item():.3e} -- L_tvs: {loss_tvs.item():.3e} ")
            
        lr_scheduler.step()    
            
        toc = ti.time()
        
        logger.info(f"----- Epoch [{ep+1:4d}]/[args.epoch:4d] -- Loss: {writer_loss/ Batch_group:.3e} -- L_nuclr: {writer_l_nuclr/ Batch_group:.3e} -- L_tvs: {writer_l_tvs/ Batch_group:.3e} -- Time: {toc - tic:.4f}")
        print('#######################')
